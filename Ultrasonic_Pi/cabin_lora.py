...
def main(train_id="T01"):
    ...
    # normal live-send timer
    next_live_tx_time = time.monotonic()

    # track reconnect events
    was_lora_down = (
        (not lora_serial) or
        (hasattr(lora_serial, "is_open") and not lora_serial.is_open)
    )

    # only rewrite SQLite when latest payload actually changes
    last_cached_payload = None
    ...
    try:
        ...
        while True:
            ...
            # 1. Keep LoRa connection updated frequently
            if (not lora_serial) or (hasattr(lora_serial, "is_open") and not lora_serial.is_open):
                lora_serial = lora_helper.connect_lora()

            lora_connected = (
                lora_serial is not None and
                (not hasattr(lora_serial, "is_open") or lora_serial.is_open)
            )

            # 2. Update latest camera states
            camera_helper.drain_camera_queue(mqtt_queue, camera_states)

            # 3. Sensor health snapshot
            sonar_ok = 1 if sensor.is_healthy() else 0
            sonar_status = sensor.get_health_status()
            seat_status = sensor.get_latest_status() if sonar_ok else -1
            distance = sensor.get_latest_distance()

            # 4. Camera health snapshot
            cam_state = camera_helper.get_effective_camera_state(
                camera_states,
                PRIMARY_CAMERA_ID,
                camera_helper.CAMERA_STALE_SECONDS
            )
            cam_ok = cam_state["cam_ok"]
            cam_status = cam_state["cam_status"]
            cam_data = cam_state["data"]
            ...
            if sonar_ok == 1 and cam_ok == 1:
                ...
                if seat_status == 0:
                    ...
                    final_seat1_status = "TAKEN" if seat1_status else "OBJECT"
            ...
            # 5. Cache latest only when payload changes
            if msg != last_cached_payload:
                msg_id = str(uuid.uuid4())[:8]
                sqlite_helper.cache_message(msg_id, msg)
                last_cached_payload = msg

                print("=" * 10)
                print(f"[CAMERA STATES] {camera_helper.summarize_camera_states(camera_states)}")
                print(f"[SONAR] ok={sonar_ok} status={sonar_status} distance={distance:.2f}cm" if distance >= 0 else f"[SONAR] ok={sonar_ok} status={sonar_status} distance=INVALID")
                print(f"[PRIMARY CAMERA] id={cam_id} ok={cam_ok} status={cam_status}")
                print(f"[CACHE] Latest state updated: {msg}")
                print(f"   L Final Mode: {mode}")
                print(f"   L Final Seat 1 Status: {final_seat1_status}")
                print(f"   L Pending now: {sqlite_helper.count_pending_messages()}")

            # 6. If LoRa just came back, send cached latest immediately
            if lora_connected and was_lora_down:
                pending = sqlite_helper.get_one_pending_message()
                if pending is not None:
                    outbound_id, outbound_msg = pending
                    print(f"[RECONNECT] Sending cached packet immediately: {outbound_msg}")

                    ok = lora_helper.try_send_lora(lora_serial, outbound_msg)
                    if ok:
                        sqlite_helper.delete_cached_message(outbound_id)
                        if outbound_msg == last_cached_payload:
                            last_cached_payload = None
                        print(f"[RECONNECT SEND OK] Message {outbound_id} sent and removed from SQLite cache.")
                        next_live_tx_time = now + TX_INTERVAL_SECONDS
                    else:
                        print(f"[RECONNECT SEND FAIL] Message {outbound_id} kept in SQLite cache.")
                        try:
                            lora_serial.close()
                        except Exception:
                            pass
            ...
            # 7. Otherwise do normal live send every 20 seconds
            elif lora_connected and now >= next_live_tx_time:
                print(f"\n[{TX_INTERVAL_SECONDS}s Cycle] Transmitting to Station: {msg}")
                print(f"   L Final Mode: {mode}")
                print(f"   L Final Seat 1 Status: {final_seat1_status}")
                ...
                if ok:
                    pending = sqlite_helper.get_one_pending_message()
                    if pending is not None:
                        sqlite_helper.delete_cached_message(pending[0])
                    last_cached_payload = None
                    print("[LIVE SEND OK] Latest state delivered.")
                    next_live_tx_time = now + TX_INTERVAL_SECONDS
                else:
                    print("[LIVE SEND FAIL] Latest state remains cached.")
                    try:
                        lora_serial.close()
                    except Exception:
                        pass
