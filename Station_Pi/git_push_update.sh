...
BRANCH="LoRa"
REPO_SUBFOLDER="Station_Pi"
# Use the directory where the script is located as the source folder
LOCAL_SOURCE_FOLDER="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
TEMP_REPO_DIR="/home/stationpi/Desktop/Git_Update_Stage"

echo "--- Starting Full Station_Pi Push Process ---"
...
git clone -b "$BRANCH" "$REPO_URL" "$TEMP_REPO_DIR"
...
# We must navigate to the destination before copy or handle paths carefully
# Using recursive copy to overwrite and add new files
cp -r "$LOCAL_SOURCE_FOLDER/"* "$DESTINATION/"

# 3. Commit and Push
cd "$TEMP_REPO_DIR"

# Configure Git Identity for this transaction
git config user.email "stationpi-update@local"
git config user.name "Station Pi User"
...
if [ -z "$status" ]; then
    ...
    echo "Committing..."
    git commit -m "Update Station_Pi folder with latest local code"
